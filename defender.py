...
def clone_correct_repo(sample_index=0):
    """
    Load a sample from SWE-bench, clone the repo, checkout the buggy commit, and apply the fix.
    
    Args:
        sample_index: Index of the sample to use from the dataset (default: 0)
    """
    # 1. Load the dataset
    print("Loading dataset...")
    sbf = load_dataset('SWE-bench/SWE-bench_verified')
    sample = sbf['test'][sample_index]
